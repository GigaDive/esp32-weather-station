...
void callBrightSkyApi(Date date) {
    ...
    http.useHTTP10(true); // HTTP1.1s chunked transfer does not work when reading a byte stream
...
void syncDWDForecast() {
    ...
    forecastData.todayIcon = iconTypeToMeteocon(
            brightskyResponse.weather_item_icon[timeinfo.tm_hour]); // Get the icon for the current hour
